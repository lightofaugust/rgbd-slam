...
// 给定index，读取一帧数据
void readFrame(int index, ParameterReader& pd, FRAME & f);
bool readFrame_kinect(VideoCapture &capture, int index, FRAME & f);
...
//    int endIndex    =   atoi( pd.getData( "end_index"   ).c_str() );
    ...
    if( !capture.isOpened() )
    {
        cout << "Kinect is not working." << endl;
        return -1;
    }
    ...
    FRAME currFrame;
    //readFrame( currIndex, pd, currFrame); // 上一帧数据
    if (readFrame_kinect(capture, 0, currFrame) == false)
    {
        cout << "Kinect is not working." << endl;
        return -1;
    }
    ...
    *******************************/
    ...
    int frame_number  =   atoi( pd.getData( "frame_number" ).c_str() );
    for (int currIndex=1; currIndex< frame_number; currIndex++ )
    {
        cout<<"Reading files "<<currIndex<<endl;
        FRAME currFrame;
        //readFrame( currIndex,pd,currFrame); // 读取currFrame
        if (readFrame_kinect(capture, currIndex, currFrame) == false)
        {
            cout << "Kinect is not working." << endl;
            return -1;
        }
    ...
    globalOptimizer.optimize( 100 ); //可以指定优化步数  //how to determine this number?
...
bool readFrame_kinect(VideoCapture & capture, int index, FRAME & f)
{
    ...
    capture.grab();
    if ( !( capture.retrieve( f.depth, CV_CAP_OPENNI_DEPTH_MAP ) ) ||
        !( capture.retrieve( f.rgb, CV_CAP_OPENNI_BGR_IMAGE ) ) )
    return false;
...
void checkNearbyLoops( vector<FRAME>& frames, FRAME& currFrame, g2o::SparseOptimizer& opti )
{
    ...
    if ( frames.size() <= nearby_loops )
    {
        ...
        for (size_t i=0; i<frames.size()-1; ++i) //because last keyframe was already checked, so here minus 1
        {
    ...
    {
        // check the nearest ones       //because last keyframe was already checked, so here minus 1
        for (size_t i = frames.size()-nearby_loops; i<frames.size()-1; ++i)
        {
            checkKeyframes( frames[i], currFrame, opti, true );
        }
    }
